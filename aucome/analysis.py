...
def analysis_on_group(group_name, groups, config_data, pvclust, verbose):
    ...
    if not os.path.isdir(group_analysis_path):
        ...
        padmet_to_padmet.padmet_to_padmet(",".join(all_padmet_path), group_analysis_path + '/' + group_name + 'panmetabolism.padmet')
        sbmlGenerator.padmet_to_sbml(padmet=group_analysis_path + '/' + group_name + '_panmetabolism.padmet', output=group_analysis_path + '/panmetabolism.sbml', verbose=verbose)
