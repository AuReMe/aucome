...
def main():
    ...
    """
    run_id = args["--run_id"]
    global database_path, studied_organisms_path, model_data_path, orthology_based_path, annotation_based_path,\
    ...
    all_study_name = sorted(os.walk(studied_organisms_path).next()[1])
    all_model_name = sorted(os.walk(model_organisms_path).next()[1])
...
def create_config_file(config_file_path):
    ...
    config.set('RUN_PATHS', 'run_id', 'template')
